...
//
// Checks on the portable parametric EQ core.  No Apple frameworks and no audio
// device: this builds paraeq_core.cpp on its own, which is the point -- the core
// is meant to go upstream beside declick and dehum, and anything it needs from
// the host belongs in the wrapper instead.
//
...
static double measuredDb(Params p, double hz)
{
    ...
    for (size_t n = 0; n < warm + meas; n += buf.size()) {
        for (size_t i = 0; i < buf.size(); i++) {
            buf[i] = (float)std::sin(2.0 * M_PI * hz * (double)(n + i) / kFs);
...
int main()
{
    ...
    std::printf("\n== a control move glides, settles, and does not click\n");
    {
        ...
        for (size_t i = 0; i < buf.size(); i++) {
            buf[i] = (float)(0.5 * std::sin(2.0 * M_PI * 200.0 * (double)i / kFs));
        }
    ...
    std::printf("\n== interleaved stride is honoured\n");
    {
        ...
        for (size_t i = 0; i < 2048; i++) {
            stereo[i * 2 + 0] = (float)std::sin(2.0 * M_PI * 10000.0 * (double)i / kFs);
            stereo[i * 2 + 1] = 0.0f;
        }
